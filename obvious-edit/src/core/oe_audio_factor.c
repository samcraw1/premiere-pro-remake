                 ...
                 gint32 track_pan, gint32 mute_or_solo_zero, gint32 factor_out[2])
{
  ...
  const gint64 pan_pair[2] = { 2 * (OE_AUDIO_UNITY - (gint64) clip_pan), 2 * (gint64) clip_pan };
  const gint64 track_pair[2]
      = { 2 * (OE_AUDIO_UNITY - (gint64) track_pan), 2 * (gint64) track_pan };
  ...
  for (gint ch = 0; ch < 2; ch++)
    {
      const gint64 product
          = (gint64) fade * clip_gain * pan_pair[ch] * track_volume * track_pair[ch];
